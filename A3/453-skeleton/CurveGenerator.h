...
public:
	...
	void GenerateBSplineCurve(const CPU_Geometry& points, CPU_Geometry& curve); // generates a quadratic b-spline curve from user created control points
...
private:
	...
	std::vector<glm::vec3> chaikin(const std::vector<glm::vec3>& points); // chaikin's algorithm for b-spline curve taken from the lecture notes
};
